...
//     NTSTATUS status = NtSetInformationProcess(GetCurrentProcess(), 0x1D, &isCritical, sizeof(ULONG));
//     if (status != 0)
//     {
...
int RunBlueScreenOfDeath2() 
{
    ...
    LPVOID lpFuncAddress = reinterpret_cast<LPVOID>(GetProcAddress(LoadLibraryW(L"ntdll.dll"), "RtlAdjustPrivilege"));
    LPVOID lpFuncAddress2 = reinterpret_cast<LPVOID>(GetProcAddress(GetModuleHandleW(L"ntdll.dll"), "NtRaiseHardError"));
    ...
    NtCall2(static_cast<long>(STATUS_FLOAT_MULTIPLE_FAULTS), 0, 0, NULL, 6, &uResp);
